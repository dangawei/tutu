...
const Util = {
  ...
  getCurrentDateTime: function(curServerTime) {
    ...
    return this.dateToStringWithYear(new Date(parseInt(curServerTime, 10)))
  },
  ...
  secondsToTimestr: function(seconds, hasHour) {
    ...
    // 得到小时
    hh = seconds / 3600 | 0
    seconds = parseInt(seconds, 10) - hh * 3600
...
const Arr = {
   ...
   * @param  {[type]} arr1
   * @param  {[type]} arr2
   * @param  {Boolean} checked: true获取差集, false获取并集
   * @return {[type]} [description]
   */
  getFilterColumns: (arr1, arr2, checked = true) => {
  	const a = new Set(arr1);
  	const b = new Set(arr2);
  	return checked ? Array.from(new Set([...a].filter(x => !b.has(x)))) : Array.from(new Set([...a, ...b]))
  },
}
